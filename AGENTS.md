...
| **工具链解析（持久化）** | 在工具调用处现场 `shutil.which("ffmpeg")` 搜 PATH，或绕过 `init_toolchain` 直接裸调 `ffmpeg`/`ffprobe` | 启动目录一变、或裸脚本（未经 `main()`）未加载 `.env.local` 时 PATH 里没有 `tools/ffmpeg/bin`，流水线前半段转写完、后半段验证/分离突然报 `ffmpeg not found`，已多次复现 | 所有工具调用**统一从持久化配置** `_GLOBAL_TOOLCHAIN.ffmpeg_path/ffprobe_path` 取绝对路径（启动时由 `.env.local` 的 `VT_FFMPEG_DIR` 解析一次并缓存，且不随 CWD 漂移）；`ffmpeg_utils._resolve_binary` 已强制走该配置，**禁止再散落 `shutil.which`**。详见 [TOOLCHAIN.md](TOOLCHAIN.md) §工具链查找 / `toolchain.init_toolchain`。 |
