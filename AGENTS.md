...
| **整段全大写** | 看到字幕/文本**整段全大写**（喊叫、高能量段，如 `st` 视频后半段）就手工改写文本，或以为模型/配置坏了反复重跑排查 | 手工改文本破坏 index 对齐且不被追溯；下次重跑仍复现（这是 Whisper 在高能量语音上的输出伪影，不是配置问题） | 转写层 `transcribe._normalize_caps` 已自动处理：**整段字母全大写且 ≥4 字母**才触发，小写化并恢复句首大写；短 token（`OK`/`ID`/`TV`/`AI`）与混合大小写段一律不动。只改文本、不碰时间戳。**全片重跑自动生效，不要手工改文本**。详见 [Spec 01](docs/specs/01-segment-schema.md)。 |
