...
- 状态：接受（**已被 ADR-034 部分降级**：音频画像不再驱动 VAD 路由，见 ADR-034 §1.2；
  本 ADR 的「VAD 选开 / 默认裸跑」语义仍然成立）
- 日期：2026-08-12
...
二者叠加才是低信噪比漏字的真正根因（见 [Spec 16](../specs/../specs/16-fill-gaps.md)）。
...
- 裸跑 + `no_speech_threshold=0.0` + 温度回退（V10）已能覆盖绝大多数场景；残留
  空洞由 [Spec 16](../specs/16-fill-gaps.md) 的强制解码兜底，无需 VAD 前置切割。
...
- 文档：README / AGENTS.md 同步标注「VAD 选开、默认裸跑（V10）」；旧文档里
  `--no-vad` 写法已纠正为 `--vad`（见 [Spec 16](../specs/../specs/16-fill-gaps.md) 备注）。
