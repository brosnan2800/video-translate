...
2. **按句末标点切句**：`。 ？ ！ … . ? !`（含全角）。
   ...
   - **无标点兜底**：若一整段无句末标点，按**软上限**强制切（`0.6 × DEFAULT_MAX_DUR` 或累计字符超 `2 × DEFAULT_MAX_CHARS`），防止"整篇一句"。
3. **时间戳插值**：合并后的句子，其 `start` 取**首片段**的 `start`，`end` 取**末片段的 `start + duration`**；句子**内部**若将来需要更细粒度，按字符长度比例分配。
