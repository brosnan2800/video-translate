...
本通路有**两个入口**，产物同契约、落到**同一目录**（`videos/<video_id>/`，故缓存与幂等续跑互通），
差异只在编排：

| | `captions <url>`（显式） | `pipeline <url>`（自动判定） |
|---|---|---|
| 定位 | **直达入口** —— 探轨道 / 强制重取 / 直连排查 | **唯一入口** —— 幂等推进整个流程 |
| 入口判定 | 使用者显式选择 | 控制平面按输入形态自动选（[Spec 24](24-pipeline-behavior.md) §6） |
| 决策点 | **不经**（直接到翻译停点） | **经**（仅选风格；无本地音频故跳过画像） |
| `--list` / `--refresh` / `--no-auto` / `--lang` | ✅ 可用 | ❌ 不经 `pipeline`；需要时改用显式 `captions` |
| 收尾 | 打印 NEXT 指向 `translate` | 同上 |

> **为什么两个都留**（ADR-042 D1 + ADR-043 D4）：`pipeline` 的自动判定消除「Agent 必须自己
> 挑入口」的负担；而 `captions` 的**直达**能力（探轨道、强制重取、不经编排直接排查通路）
> 是自动路由替代不了的 —— 尤其 ADR-042 D8 的通路失败需要快速定位病因时。
