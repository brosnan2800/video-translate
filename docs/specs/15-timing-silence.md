...
- 修复：词级时间戳 + **`merge.py` 自写的 `_split_by_gap`** 在**静音处切开**长段，让字幕体现真实停顿（Spec 12 + 13 的词级数据支撑）。
  > 注：`stable-ts` 是早期方案里的依赖，已被 [ADR-008](../adr/008-stable-ts-spike-rejected.md) **拒收**——
  > 只保留「词级时间戳」这一能力（走 faster-whisper 原生），切分逻辑自写、不引入该库。
